-- ============================================================================
-- BusNStay Town Order Management & Restaurant Notification System
-- Database Migration Script for Supabase
-- ============================================================================

-- ============= ENUMS / TYPES =============

CREATE TYPE town_status AS ENUM ('open', 'closed', 'locked');
CREATE TYPE order_status AS ENUM ('pending', 'accepted', 'preparing', 'ready', 'completed', 'cancelled');
CREATE TYPE notification_channel AS ENUM ('inApp', 'whatsApp', 'sms', 'email');

-- ============= JOURNEYS & TOWNS =============

-- Journeys table (bus routes)
CREATE TABLE IF NOT EXISTS public.journeys (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  bus_id TEXT NOT NULL,
  route_name TEXT NOT NULL,
  departure_time TIMESTAMPTZ NOT NULL,
  estimated_arrival_time TIMESTAMPTZ NOT NULL,
  current_latitude DOUBLE PRECISION DEFAULT 0.0,
  current_longitude DOUBLE PRECISION DEFAULT 0.0,
  last_position_update TIMESTAMPTZ DEFAULT NOW(),
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  CONSTRAINT valid_coordinates CHECK (
    current_latitude BETWEEN -90 AND 90 AND
    current_longitude BETWEEN -180 AND 180
  )
);

CREATE INDEX idx_journeys_bus_id ON public.journeys(bus_id);
CREATE INDEX idx_journeys_is_active ON public.journeys(is_active);
CREATE INDEX idx_journeys_created_at ON public.journeys(created_at);

-- Journey towns (stops along the route)
CREATE TABLE IF NOT EXISTS public.journey_towns (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  journey_id TEXT NOT NULL REFERENCES public.journeys(id) ON DELETE CASCADE,
  town_id TEXT NOT NULL,
  town_name TEXT NOT NULL,
  latitude DOUBLE PRECISION NOT NULL,
  longitude DOUBLE PRECISION NOT NULL,
  pickup_station_name TEXT NOT NULL,
  estimated_stop_duration_seconds INTEGER DEFAULT 600,
  
  -- Status management
  status town_status DEFAULT 'open',
  status_changed_at TIMESTAMPTZ,
  eta_to_town INTEGER, -- In seconds
  distance_to_town DOUBLE PRECISION, -- In kilometers
  
  -- Configuration
  order_cutoff_before_minutes INTEGER DEFAULT 10,
  order_cutoff_distance_km DOUBLE PRECISION DEFAULT 3.0,
  
  metadata JSONB,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  
  CONSTRAINT valid_town_coordinates CHECK (
    latitude BETWEEN -90 AND 90 AND
    longitude BETWEEN -180 AND 180
  ),
  CONSTRAINT valid_distance CHECK (distance_to_town >= 0),
  UNIQUE(journey_id, town_id)
);

CREATE INDEX idx_journey_towns_journey_id ON public.journey_towns(journey_id);
CREATE INDEX idx_journey_towns_status ON public.journey_towns(status);
CREATE INDEX idx_journey_towns_town_id ON public.journey_towns(town_id);

-- ============= ORDERS =============

-- Food orders (enhanced schema)
CREATE TABLE IF NOT EXISTS public.orders (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  
  -- Customer information
  customer_id TEXT NOT NULL,
  customer_name TEXT NOT NULL,
  customer_phone TEXT NOT NULL,
  
  -- Restaurant and journey information
  restaurant_id TEXT NOT NULL,
  restaurant_name TEXT NOT NULL,
  journey_id TEXT NOT NULL REFERENCES public.journeys(id) ON DELETE CASCADE,
  town_id TEXT NOT NULL REFERENCES public.journey_towns(town_id),
  town_name TEXT NOT NULL,
  
  -- Order content
  items JSONB NOT NULL, -- Array of {name, quantity, price, specialRequest}
  special_instructions TEXT,
  
  -- Pricing
  subtotal DECIMAL(10, 2) NOT NULL,
  delivery_fee DECIMAL(10, 2) DEFAULT 0,
  platform_fee DECIMAL(10, 2) DEFAULT 0,
  total_amount DECIMAL(10, 2) NOT NULL,
  currency TEXT DEFAULT 'ZMW',
  
  -- Payment & Status
  status order_status DEFAULT 'pending',
  payment_confirmed_at TIMESTAMPTZ,
  estimated_bus_arrival_time TIMESTAMPTZ,
  restaurant_notified BOOLEAN DEFAULT false,
  restaurant_notified_at TIMESTAMPTZ,
  
  -- Delivery information
  pickup_address TEXT,
  delivery_address TEXT,
  
  -- Timestamps
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  
  CONSTRAINT total_matches_calculation CHECK (
    total_amount = subtotal + COALESCE(delivery_fee, 0) + COALESCE(platform_fee, 0)
  )
);

CREATE INDEX idx_orders_customer_id ON public.orders(customer_id);
CREATE INDEX idx_orders_restaurant_id ON public.orders(restaurant_id);
CREATE INDEX idx_orders_journey_id ON public.orders(journey_id);
CREATE INDEX idx_orders_town_id ON public.orders(town_id);
CREATE INDEX idx_orders_status ON public.orders(status);
CREATE INDEX idx_orders_created_at ON public.orders(created_at);
CREATE INDEX idx_orders_payment_confirmed ON public.orders(payment_confirmed_at);

-- ============= RESTAURANT NOTIFICATIONS =============

-- Restaurant notifications (audit trail for order notifications)
CREATE TABLE IF NOT EXISTS public.restaurant_notifications (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  restaurant_id TEXT NOT NULL,
  order_id TEXT NOT NULL REFERENCES public.orders(id) ON DELETE CASCADE,
  
  -- Message content
  message TEXT NOT NULL,
  notification_type TEXT NOT NULL, -- 'order_placed', 'order_ready', 'pickup_ready'
  order_details JSONB, -- Full order snapshot
  town_name TEXT,
  estimated_arrival_minutes INTEGER,
  
  -- Status tracking
  read BOOLEAN DEFAULT false,
  read_at TIMESTAMPTZ,
  acknowledged BOOLEAN DEFAULT false,
  acknowledged_at TIMESTAMPTZ,
  
  -- Timestamps
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  
  CONSTRAINT valid_type CHECK (
    notification_type IN ('order_placed', 'order_ready', 'pickup_ready', 'order_cancelled')
  )
);

CREATE INDEX idx_restaurant_notifications_restaurant_id 
  ON public.restaurant_notifications(restaurant_id);
CREATE INDEX idx_restaurant_notifications_order_id 
  ON public.restaurant_notifications(order_id);
CREATE INDEX idx_restaurant_notifications_read 
  ON public.restaurant_notifications(read);
CREATE INDEX idx_restaurant_notifications_created_at 
  ON public.restaurant_notifications(created_at);

-- Notification delivery tracking
CREATE TABLE IF NOT EXISTS public.notification_deliveries (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  notification_id TEXT NOT NULL REFERENCES public.restaurant_notifications(id) ON DELETE CASCADE,
  channel notification_channel NOT NULL,
  
  recipient_phone TEXT,
  recipient_email TEXT,
  
  sent BOOLEAN DEFAULT false,
  sent_at TIMESTAMPTZ,
  delivery_status TEXT, -- 'sent', 'delivered', 'failed', 'bounced'
  error_message TEXT,
  
  retry_count INTEGER DEFAULT 0,
  last_retry_at TIMESTAMPTZ,
  
  external_message_id TEXT, -- From Twilio, Wati, etc
  
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_notification_deliveries_notification_id 
  ON public.notification_deliveries(notification_id);
CREATE INDEX idx_notification_deliveries_channel 
  ON public.notification_deliveries(channel);
CREATE INDEX idx_notification_deliveries_sent_at 
  ON public.notification_deliveries(sent_at);

-- Notification audit log
CREATE TABLE IF NOT EXISTS public.notification_audit_log (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  notification_id TEXT,
  order_id TEXT REFERENCES public.orders(id),
  restaurant_id TEXT NOT NULL,
  action TEXT NOT NULL, -- 'sent', 'failed', 'retried', 'acknowledged'
  channel notification_channel,
  details JSONB,
  timestamp TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_notification_audit_restaurant_id 
  ON public.notification_audit_log(restaurant_id);
CREATE INDEX idx_notification_audit_timestamp 
  ON public.notification_audit_log(timestamp);

-- ============= TOWN STATUS UPDATES =============

-- Real-time town status change tracking
CREATE TABLE IF NOT EXISTS public.town_status_updates (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  journey_id TEXT NOT NULL REFERENCES public.journeys(id) ON DELETE CASCADE,
  town_id TEXT NOT NULL,
  town_name TEXT NOT NULL,
  
  old_status town_status, -- Previous status
  new_status town_status NOT NULL, -- New status
  reason TEXT, -- 'auto_closed', 'manual_close', 'bus_passed', 'reopened'
  
  changed_at TIMESTAMPTZ DEFAULT NOW(),
  triggered_by TEXT, -- 'system', 'bus_operator', 'admin'
  
  created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_town_status_updates_journey_id 
  ON public.town_status_updates(journey_id);
CREATE INDEX idx_town_status_updates_created_at 
  ON public.town_status_updates(created_at);

-- ============= ANALYTICS & REPORTING =============

-- Order analytics view
CREATE VIEW public.order_analytics AS
SELECT
  j.id as journey_id,
  j.route_name,
  j.departure_time,
  jt.town_name,
  COUNT(o.id) as total_orders,
  COUNT(CASE WHEN o.restaurant_notified THEN 1 END) as notified_orders,
  SUM(o.total_amount) as total_revenue,
  AVG(EXTRACT(EPOCH FROM (o.created_at - j.departure_time))) as avg_order_time_seconds,
  COUNT(CASE WHEN o.status = 'completed' THEN 1 END) as completed_orders
FROM public.journeys j
LEFT JOIN public.journey_towns jt ON j.id = jt.journey_id
LEFT JOIN public.orders o ON jt.town_id = o.town_id AND j.id = o.journey_id
GROUP BY j.id, j.route_name, j.departure_time, jt.town_name;

-- ============= ROW LEVEL SECURITY =============

-- Enable RLS
ALTER TABLE public.journeys ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.journey_towns ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.orders ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.restaurant_notifications ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.notification_deliveries ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.town_status_updates ENABLE ROW LEVEL SECURITY;

-- Allow bus operators to see their journeys
CREATE POLICY "bus_operators_can_view_own_journeys" ON public.journeys
FOR SELECT USING (
  auth.uid()::text = bus_id OR
  EXISTS (
    SELECT 1 FROM public.user_profiles
    WHERE id = auth.uid()::text
    AND user_role = 'bus_operator'
  )
);

-- Allow restaurants to see notifications for their orders
CREATE POLICY "restaurants_can_see_own_notifications" ON public.restaurant_notifications
FOR SELECT USING (
  auth.uid()::text = restaurant_id OR
  EXISTS (
    SELECT 1 FROM public.user_profiles
    WHERE id = auth.uid()::text
    AND user_role = 'restaurant_owner'
  )
);

-- Allow customers to see their orders
CREATE POLICY "customers_can_see_own_orders" ON public.orders
FOR SELECT USING (
  auth.uid()::text = customer_id
);

-- ============= FUNCTIONS =============

-- Function to auto-close towns based on bus proximity
CREATE OR REPLACE FUNCTION public.auto_close_nearby_towns()
RETURNS void AS $$
DECLARE
  v_journey RECORD;
  v_town RECORD;
  v_distance FLOAT8;
  v_eta_minutes INT;
BEGIN
  FOR v_journey IN
    SELECT * FROM public.journeys WHERE is_active = true
  LOOP
    FOR v_town IN
      SELECT * FROM public.journey_towns
      WHERE journey_id = v_journey.id
      AND status = 'open'
    LOOP
      -- Calculate distance using Haversine formula
      v_distance := (
        6371 * acos(
          cos(radians(90 - v_journey.current_latitude)) *
          cos(radians(90 - v_town.latitude)) +
          sin(radians(90 - v_journey.current_latitude)) *
          sin(radians(90 - v_town.latitude)) *
          cos(radians(v_journey.current_longitude - v_town.longitude))
        )
      );

      -- Estimate ETA (assuming 80 km/h average)
      v_eta_minutes := (v_distance / 80.0) * 60;

      -- Close if within cutoff
      IF v_distance <= v_town.order_cutoff_distance_km
         OR v_eta_minutes <= v_town.order_cutoff_before_minutes
      THEN
        UPDATE public.journey_towns
        SET
          status = 'closed',
          distance_to_town = v_distance,
          eta_to_town = (v_eta_minutes * 60)::INTEGER,
          status_changed_at = NOW(),
          updated_at = NOW()
        WHERE id = v_town.id;

        INSERT INTO public.town_status_updates
        (journey_id, town_id, town_name, old_status, new_status, reason, triggered_by)
        VALUES
        (v_journey.id, v_town.town_id, v_town.town_name, 'open', 'closed', 'auto_closed', 'system');
      END IF;
    END LOOP;
  END LOOP;
END;
$$ LANGUAGE plpgsql;

-- Function to notify restaurant when order is placed
CREATE OR REPLACE FUNCTION public.notify_restaurant_on_order()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.restaurant_notified = false AND NEW.payment_confirmed_at IS NOT NULL THEN
    INSERT INTO public.restaurant_notifications
    (restaurant_id, order_id, message, notification_type, order_details, town_name, estimated_arrival_minutes)
    VALUES (
      NEW.restaurant_id,
      NEW.id,
      'New order from ' || NEW.customer_name || ' for delivery to ' || NEW.town_name,
      'order_placed',
      row_to_json(NEW),
      NEW.town_name,
      EXTRACT(EPOCH FROM (NEW.estimated_bus_arrival_time - NOW())) / 60
    );

    UPDATE public.orders
    SET restaurant_notified = true, restaurant_notified_at = NOW()
    WHERE id = NEW.id;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trigger_notify_restaurant_on_order
AFTER INSERT OR UPDATE ON public.orders
FOR EACH ROW
EXECUTE FUNCTION public.notify_restaurant_on_order();

-- ============= GRANT PERMISSIONS =============

GRANT SELECT, INSERT, UPDATE ON public.journeys TO authenticated;
GRANT SELECT, INSERT, UPDATE ON public.journey_towns TO authenticated;
GRANT SELECT, INSERT, UPDATE ON public.orders TO authenticated;
GRANT SELECT, INSERT ON public.restaurant_notifications TO authenticated;
GRANT SELECT, INSERT ON public.notification_deliveries TO authenticated;
GRANT SELECT ON public.notification_audit_log TO authenticated;
GRANT SELECT ON public.town_status_updates TO authenticated;
GRANT SELECT ON public.order_analytics TO authenticated;

-- ============= DATA INITIALIZATION =============

-- Insert sample data for testing (optional)
-- This should be run separately after tables are created
