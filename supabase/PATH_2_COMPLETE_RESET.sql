-- ==================== BUSNSTAY COMPLETE FRESH START ====================
-- PATH 2: Complete Reset with proper extension setup
-- This enables all required extensions first, then builds fresh

-- ==================== STEP 1: ENABLE EXTENSIONS ====================
-- CRITICAL: These must exist before creating tables
...
DROP TABLE IF EXISTS public.room_rate_history CASCADE;
...
DROP TABLE IF EXISTS public.restaurant_approval_logs CASCADE;
DROP TABLE IF EXISTS public.accommodation_bookings CASCADE;
DROP TABLE IF EXISTS public.accommodations CASCADE;
DROP TABLE IF EXISTS public.shared_journey_links CASCADE;
DROP TABLE IF EXISTS public.route_performance_history CASCADE;
DROP TABLE IF EXISTS public.journey_alerts CASCADE;
DROP TABLE IF EXISTS public.orders CASCADE;
DROP TABLE IF EXISTS public.menu_items CASCADE;
DROP TABLE IF EXISTS public.restaurants CASCADE;
DROP TABLE IF EXISTS public.delivery_agents CASCADE;
DROP TABLE IF EXISTS public.journey_etas CASCADE;
DROP TABLE IF EXISTS public.gps_history CASCADE;
DROP TABLE IF EXISTS public.journey_passengers CASCADE;
...
CREATE TABLE public.restaurant_approval_logs (
...
-- Room rate history
CREATE TABLE public.room_rate_history (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  room_id UUID NOT NULL REFERENCES hotel_rooms(id) ON DELETE CASCADE,
...
CREATE INDEX idx_restaurants_approval_status ON public.restaurants(approval_status);
CREATE INDEX idx_restaurants_is_approved ON public.restaurants(is_approved);
CREATE INDEX idx_restaurants_location ON public.restaurants USING GIST(location);
CREATE INDEX idx_accommodations_location ON public.accommodations USING GIST(location);
...
CREATE INDEX idx_accommodation_bookings_user ON public.accommodation_bookings(user_id);
CREATE INDEX idx_restaurant_approval_logs_restaurant_id ON restaurant_approval_logs(restaurant_id);
CREATE INDEX idx_hotel_rooms_accommodation ON hotel_rooms(accommodation_id);
...
CREATE INDEX idx_room_rate_history_room ON room_rate_history(room_id);
...
ALTER TABLE public.restaurant_approval_logs ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE public.room_rate_history ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Anyone can view hotel rooms" ON public.hotel_rooms FOR SELECT USING (true);
CREATE POLICY "Anyone can view room reviews" ON public.room_reviews FOR SELECT USING (true);
...
CREATE POLICY "Users see own payment transactions" ON public.payment_transactions
FOR SELECT USING (auth.uid() = user_id);
...
CREATE TRIGGER trigger_update_room_rating
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.update_room_average_rating();
